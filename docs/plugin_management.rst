...
The Wooting SDK requires system plugins to communicate with keyboards.
This guide explains how to install, verify, and troubleshoot the plugin
installation.
...
3. Automatically runs the complete setup (permissions → interface →
   plugins)
...
This command runs: - permission setup (Linux/macOS) - CFFI interface
build (if missing) - SDK/plugin installation - macOS Gatekeeper fixups
...
Remove interface, plugins, and all system files (for complete
reinstall):
...
**Common causes**: 1. Keyboard not connected via USB 2. Plugins not
installed 3. Permissions issues (Linux) 4. Version mismatch between
wrapper and SDK
...
The Gatekeeper setup should handle code signing automatically, but if
you encounter this:
...
**Problem**: User is not in the ``input`` group or udev rules are not
applied
